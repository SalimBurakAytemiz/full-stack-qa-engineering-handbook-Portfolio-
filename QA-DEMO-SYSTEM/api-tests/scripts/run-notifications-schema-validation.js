...
// P5.6 — AJV / JSON Schema Validation runner for the Notifications
// collection.
//
...
//
// Scope: the empty-list baseline response (proves the empty-array case is
// schema-valid), the populated response right after the PAID-order gate
// (proves a real notification item is schema-valid — type enum, is_read as
// integer not boolean, no user_id leakage field), the folder-2 positive
// contract response (independent re-check), and the no-token 401 error
// response (proves the reused common/error-response schema holds here too,
// consistent with how P5.5 validated it across several status codes).
...
const COLLECTION_PATH = path.join(__dirname, '..', 'postman', 'collections', 'qa-demo-system-notifications.postman_collection.json');
const ENVIRONMENT_PATH = path.join(__dirname, '..', 'postman', 'environments', 'local.postman_environment.json');

const notificationItemSchema = require(path.join(SCHEMAS_DIR, 'notifications', 'notification-item.schema.json'));
const notificationsListResponseSchema = require(path.join(SCHEMAS_DIR, 'notifications', 'notifications-list-response.schema.json'));
const errorResponseSchema = require(path.join(SCHEMAS_DIR, 'common', 'error-response.schema.json'));

const ajv = new Ajv({ strict: true, allErrors: true });
// notification-item.schema.json is added once so notifications-list-response
// can $ref it (real schema reuse, not copy/paste).
ajv.addSchema(notificationItemSchema);
...
const REQUEST_CHECKS = {
  'Capture notification baseline before PAID order (User A)': {
    validate: ajv.compile(notificationsListResponseSchema),
...
newman
  ...
  .on('done', (err, summary) => {
    ...
    if (newmanFailed || anyFailed) {
      console.error('\nP5.6 schema validation run: FAIL');
      process.exitCode = 1;
      return;
    }
